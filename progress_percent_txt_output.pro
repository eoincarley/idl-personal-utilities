pro progress_percent_txt_output,i,start_i,end_i
...
if ((i-st)*1. mod ((et-st)/10.)) lt (((i-st)*1.-1) mod ((et-st)/10.)) then begin
	if floor((i-st)*100./(et-st)) lt 100 then printf,30,floor((i-st)*100./(et-st)),'%',format='(I2,A,$)'
	if floor((i-st)*100./(et-st)) ge 100 and i eq et then begin
		printf,30,floor((i-st)*100./(et-st)),'%',format='(I3,A)'
		return
	endif
endif else if i ge et then begin
	printf,30,'100%',format='(A)'
	return
endif else if (et-st) gt 100 then if ((i-st)*1. mod ((et-st)/50.)) lt (((i-st)*1.-1) mod ((et-st)/50.)) then begin
	printf,30,'.',format='(A,$)'
endif

if (et-st) lt 100 then	printf,30,'.',format='(A,$)'
